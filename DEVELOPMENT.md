...
Docker 在本项目中不是必须项，但推荐用于团队统一开发环境。简单说：Docker 不负责运行 React 页面，也不直接替代 Node.js API；它主要负责把后端依赖服务容器化，当前最重要的是 PostgreSQL。
...
- 方便以后部署时把 API、数据库、反向代理等服务统一编排

当前项目有两种数据库运行方式，二选一：

```text
方式 A：本机 PostgreSQL
Node API -> localhost:5432 -> Windows 上安装的 PostgreSQL 服务

方式 B：Docker PostgreSQL
Node API -> localhost:5432 -> Docker 容器里的 PostgreSQL
```
...
切换到 Docker PostgreSQL 的推荐步骤：

```powershell
# 1. 停止本机 PostgreSQL，释放 5432
...
如果不想停止本机 PostgreSQL，也可以把 `docker-compose.yml` 的端口改成 `5433:5432`，然后把 `.env` 改成：

```env
DATABASE_URL="postgresql://family_graph:family_graph@localhost:5433/family_graph?schema=public"
```
